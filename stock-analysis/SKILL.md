...
1. **Snapshot** — `get_stock_snapshot` with `include_sentiment=false`, `include_broker_consensus=true`, `include_news=true`, `include_sector_metrics=true`. Captures current price, valuation multiples, latest quarterly NPATMI, broker consensus summary, and recent news.
...
3. **Quarterly financials (earnings strip)** — `mssql_read_data` on `FA_Quarterly` for the last 8 quarters. Pull: `Net_Revenue`, `Gross_Profit`, `NPAT`, `NPATMI`, `Financial_Income`, `Affiliate_Income`, `Other_Income`, `Operating_CF`. This is required for Section 6's quarterly earnings quality strip — do not skip.
...
5. **Broker consensus** — `supabase_consensus_analyze` with `comparisons=5`. Captures multi-broker TP table, NPATMI estimates, recent sentiment shifts, and catalyst tracker. This is the primary source for Sections 10 and 11.
6. **Peer market data** — `mssql_read_data` on `Market_Data` for 4–6 domestic sector peers, same-day snapshot: `PX_LAST`, `MKT_CAP`, `PE`. Use for the comps table in Section 10.
...
- Dividend yield
- 5-year average ROE
**Net cash adjustment**: If the subject company's net cash (cash + short-term investments − total debt) exceeds 25% of market cap, compute and disclose *operating EV* separately (market cap − net cash) and restate EV/EBITDA on that basis. This is the most meaningful multiple for cash-rich companies where headline EV/EBITDA materially overstates the implied price for the operating business.
